...
public class JwtUserService {
    ...
    public String tokenFromUserEntity(UserEntity entity) {
        JwtPayload jwtPayload = new JwtPayload(entity.getId(), entity.getEmail(),Instant.now().getEpochSecond() + TOKEN_LIFETIME);
        String messageToHash = base64Util.base64FromString(HEADER) + "." + base64Util.base64FromString(jwtPayload.generatePayload());
        return messageToHash.concat("." + base64Util.base64FromBytes(HmacSha256Service.hash(messageToHash, SECRET)));
    }
    ...
    public JwtPayload payloadFromToken(String token) {
        ...
        if(!split[2].equals(base64Util.base64FromBytes(HmacSha256Service.hash(split[0].concat("." + split[1]), SECRET))))
